...
public class ViewFlowCircleIndicator extends View implements ViewFlowIndicator,
	...
	public ViewFlowCircleIndicator(Context context, AttributeSet attrs) {
		...
		int activeType = a.getInt(R.styleable.CircleIndicator_activeType,
				STYLE_FILL);

		int activeDefaultColor = 0xFFFFFFFF;

		// Get a custom inactive color if there is one
		int activeColor = a.getColor(R.styleable.CircleIndicator_activeColor,
				activeDefaultColor);

		// Gets the inactive circle type, defaulting to "stroke"
		int inactiveType = a.getInt(R.styleable.CircleIndicator_inactiveType,
				STYLE_STROKE);

		int inactiveDefaultColor = 0x44FFFFFF;
		// Get a custom inactive color if there is one
		int inactiveColor = a
				.getColor(R.styleable.CircleIndicator_inactiveColor,
						inactiveDefaultColor);

		// Retrieve the radius
		radius = a.getDimension(R.styleable.CircleIndicator_radius, 4.0f);

		circleSeparation = a.getDimension(
				R.styleable.CircleIndicator_circleSeparation, 2 * radius
						+ radius);
		activeRadius = a.getDimension(R.styleable.CircleIndicator_activeRadius,
				0.5f);
		// Retrieve the fade out time
		fadeOutTime = a.getInt(R.styleable.CircleIndicator_fadeOut, 0);
	...
	public void reMeasureWidthHeight() {
		setMeasuredDimension(measureWidth(measureSpecWidth),
